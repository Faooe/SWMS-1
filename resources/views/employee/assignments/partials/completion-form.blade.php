...
<form method="POST" action="{{ route('employee.assignments.complete', $assignment->uuid) }}" enctype="multipart/form-data" class="space-y-4">
    ...
    <div class="grid grid-cols-2 gap-3">
        <label class="group block cursor-pointer rounded-2xl border border-dashed border-slate-300 bg-white p-4 text-center transition hover:border-blue-300 hover:bg-blue-50/40">
            ...
            <span class="js-completion-photo-label block text-xs font-semibold text-slate-600">Bukti 1 (wajib)</span>
            <input type="file" name="completion_photo" accept="image/*,video/*,application/pdf" capture="environment" class="js-completion-photo hidden" required>
        </label>

        <label class="group block cursor-pointer rounded-2xl border border-dashed border-slate-300 bg-white p-4 text-center transition hover:border-blue-300 hover:bg-blue-50/40">
            <i data-lucide="images" class="mx-auto mb-2 h-5 w-5 text-slate-400 transition group-hover:text-blue-500"></i>
            <span class="js-completion-photo-2-label block text-xs font-semibold text-slate-600">Bukti 2 (opsional)</span>
            <input type="file" name="completion_photo_2" accept="image/*,video/*,application/pdf" capture="environment" class="js-completion-photo-2 hidden">
        </label>
    </div>
    ...
    <div class="flex items-start gap-2 rounded-xl bg-blue-50 px-3 py-2 text-[11px] leading-relaxed text-blue-700">
        ...
        <span>Bukti wajib berupa foto, PDF, atau video. Foto di atas 200KB dan video di atas 5MB otomatis dikompres; video maksimal 1 menit.</span>
    </div>
