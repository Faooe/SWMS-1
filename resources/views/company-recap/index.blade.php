...
<div class="mx-auto max-w-[1680px] space-y-6">
    ...
    <form method="GET" action="{{ route('company-recap.index') }}" class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm lg:p-6">
        <div class="mb-5 flex items-start gap-3">
            <span class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-600"><i data-lucide="list-filter" class="h-5 w-5"></i></span>
            <div><h3 class="font-bold text-slate-900">Periode dan Filter</h3><p class="mt-0.5 text-xs text-slate-500">Pilih rentang laporan dan gunakan filter organisasi untuk mempersempit hasil.</p></div>
        </div>
        <div class="space-y-5">
            <div><p class="mb-3 text-[11px] font-extrabold uppercase tracking-[0.16em] text-blue-600">Rentang laporan</p><div class="grid gap-4 md:grid-cols-3"><div><label class="mb-1.5 block text-xs font-bold text-slate-500">Dari tanggal</label><input type="date" name="from" value="{{ $range['from'] }}" class="w-full rounded-xl border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500"></div><div><label class="mb-1.5 block text-xs font-bold text-slate-500">Sampai tanggal</label><input type="date" name="to" value="{{ $range['to'] }}" max="{{ today()->toDateString() }}" class="w-full rounded-xl border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500"></div><div><label class="mb-1.5 block text-xs font-bold text-slate-500">Cari employee</label><input type="search" name="search" value="{{ request('search') }}" placeholder="Nama atau NIP employee..." class="w-full rounded-xl border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500"></div></div></div>
