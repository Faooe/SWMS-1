...
class AttendanceCheckoutCorrectionService
{
    public function request(User $user, Assignment $assignment, string $date, string $requestedTime, string $reason): AttendanceCheckoutCorrection
    {
        ...
        if (! $employee || ! $assignment->daily_attendance_enabled || ! $assignment->employees()->where('employees.id', $employee->id)->exists()) {
            abort(403);
        }

        $attendanceDate = Carbon::parse($date)->startOfDay();
        if (! $attendanceDate->lt(today())) {
            throw ValidationException::withMessages(['date' => ['Koreksi hanya untuk attendance hari sebelumnya yang lupa Check Out.']]);
        }
        ...
        if (! $attendance || ! $attendance->is_checked_in) {
            throw ValidationException::withMessages(['attendance' => ['Koreksi Check Out tidak tersedia karena tidak ada Check In pada hari tersebut. Lupa Check In tidak dapat dikoreksi.']]);
        }
        ...
        if (! $requested->gt($checkIn) || $requested->gt($dayLimit)) {
            throw ValidationException::withMessages(['requested_check_out_time' => ['Jam Check Out harus setelah Check In dan maksimal 23:00 pada hari yang sama.']]);
        }
    ...
    public function approve(User $reviewer, Assignment $assignment, AttendanceCheckoutCorrection $correction, ?string $notes = null): AttendanceCheckoutCorrection
    {
        ...
        return DB::transaction(function () use ($reviewer, $assignment, $correction, $notes) {
            ...
            if ($attendance->is_checked_out) {
                throw ValidationException::withMessages(['attendance' => ['Attendance sudah memiliki Check Out.']]);
            }
